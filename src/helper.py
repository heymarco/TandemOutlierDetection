...
def get_point_outliers(os_ondevice, os_federated, percentile=99,
                       is_outlier_confidence=0.95, classification_confidence=0.95):
    ...
    local_outlier_candidates = os_federated < percentile_thresh_federated
    ...
    significant_local_outliers = np.logical_and(
        all_outliers,
        np.logical_and(
            local_outlier_candidates,
            confidence_federated > classification_confidence
        )
    )

    indecisive_outliers = np.logical_and(
        all_outliers,
        np.invert(
            np.logical_or(
                significant_global_outliers,
                significant_local_outliers
            )
        )
    )
    ...
    log(logging.WARNING, "Number of indecisive outliers: {}".format(np.sum(indecisive_outliers)))

    return significant_local_outliers, significant_global_outliers, indecisive_outliers
...
def server_evaluation(os_federated: np.ndarray):
    ...
    std = np.std(os_star)
    probs = [norm(mean, std).sf(val) for val in os_star]
