...
def plot_ipek():
    ...
    for filename in filenames:
        ...
        file = file[file["batC"] > 0.6]
        file.reset_index(drop=True, inplace=True)
        axes.flatten()[index].plot(file["batC"].iloc[:int(len(file)*ipek_split_ratios[index])])
        index += 1
    plt.show()
...
class IpekRawDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data, transform, labels=False, seq_len=100):
        self.data = data
    ...
    def __getitem__(self, idx):
        sample = self.data[idx*self.seq_len:idx*self.seq_len+self.seq_len]
...
def __ipek_data_loader__(index, batch_size: int = 64):
...
def load_ipek_partition(index: int):
